...
namespace mu
{
	...
	public:
		...
		//void SetWindowView(CView* cview);
		//void SetVideoView(CView* pview);
		//
		//int m_iupdatetype;/*0所有 类型都更新 1.只跟新double*/
		//void SetUpDateType(int itype){m_iupdatetype = itype;}
		void GetImageObjectAtt();
		//
		double RunOptCode();//运行Opt收集代码
		void RunFastCode();
		void SetOptCollect(bool Open_Close);//开启Opt收集
	...
	private:
		std::ostream *m_stream;//输出操作
		std::ostream *m_createstream;//属性输出操作
