...
class ImageEstimatedModuleComparator
{
public:
    bool operator()(ImageRef<ImageQRFinderPattern> a, ImageRef<ImageQRFinderPattern> b) {
        return a->getEstimatedModuleSize() < b->getEstimatedModuleSize();
    }
};


int ImageQRFinderPatternFinder::CENTER_QUORUM = 2;
int ImageQRFinderPatternFinder::MIN_SKIP = 3;
int ImageQRFinderPatternFinder::MAX_MODULES = 57;

float ImageQRFinderPatternFinder::centerFromEnd(int* stateCount, int end) {
  return (float)(end - stateCount[4] - stateCount[3]) - stateCount[2] / 2.0f;
}

bool ImageQRFinderPatternFinder::foundPatternCross(int* stateCount) {
...
float ImageQRFinderPatternFinder::crossCheckVertical(size_t startI, size_t centerJ, int maxCount, int originalStateCountTotal) {
...
float ImageQRFinderPatternFinder::crossCheckHorizontal(size_t startJ, size_t centerI, int maxCount,
...
bool ImageQRFinderPatternFinder::handlePossibleCenter(int* stateCount, size_t i, size_t j)
{
  int stateCountTotal = stateCount[0] + stateCount[1] + stateCount[2] + stateCount[3] + stateCount[4];
  ...
  if (!isnan_z(centerI))
  {
    ...
    if (!isnan_z(centerJ) && crossCheckDiagonal((int)centerI, (int)centerJ, stateCount[2], stateCountTotal)) {
      ...
      {
        ImageRef<ImageQRFinderPattern> center = possibleCenters_[index];
      ...
      if (!found) {
        ImageRef<ImageQRFinderPattern> newPattern(new ImageQRFinderPattern(centerJ, centerI, estimatedModuleSize));
...
int ImageQRFinderPatternFinder::findRowSkip() {
  size_t max = possibleCenters_.size();
  if (max <= 1) {
    return 0;
  }
  ImageRef<ImageQRFinderPattern> firstConfirmedCenter;
  for (size_t i = 0; i < max; i++) {
    ImageRef<ImageQRFinderPattern> center = possibleCenters_[i];
...
bool ImageQRFinderPatternFinder::haveMultiplyConfirmedCenters() {
  int confirmedCount = 0;
  float totalModuleSize = 0.0f;
  size_t max = possibleCenters_.size();
  for (size_t i = 0; i < max; i++) {
    ImageRef<ImageQRFinderPattern> pattern = possibleCenters_[i];
...
vector< ImageRef<ImageQRFinderPattern> > ImageQRFinderPatternFinder::selectBestPatterns() {
  std::vector<ImageRef<ImageQRFinderPattern>> bestPatterns;
...
vector<ImageRef<ImageQRFinderPattern> > ImageQRFinderPatternFinder::orderBestPatterns(vector<ImageRef<ImageQRFinderPattern> > patterns) {
  ...
  ImageRef<ImageQRFinderPattern> topLeft;
  ImageRef<ImageQRFinderPattern> topRight;
  ImageRef<ImageQRFinderPattern> bottomLeft;
  ...
  if ((bottomLeft->getY() - topLeft->getY()) * (topRight->getX() - topLeft->getX()) < (bottomLeft->getX()
    ...
    ImageRef<ImageQRFinderPattern> temp = topRight;
    topRight = bottomLeft;
    bottomLeft = temp;
  }

  vector<ImageRef<ImageQRFinderPattern> > results(3);
...
float ImageQRFinderPatternFinder::distance(ImageRef<ImageResultPoint> p1, ImageRef<ImageResultPoint> p2) {
  float dx = p1->getX() - p2->getX();
  float dy = p1->getY() - p2->getY();
  return (float)sqrt(dx * dx + dy * dy);
}

ImageQRFinderPatternFinder::ImageQRFinderPatternFinder(ImageRef<ImageBitMatrix> image,
                                           ImageRef<ImageResultPointCallback>const& callback) :
    image_(image), possibleCenters_(), hasSkipped_(false), callback_(callback) {
}

ImageRef<ImageQRFinderPatternInfo> ImageQRFinderPatternFinder::find(ImageCodeTypes const& hints) {
  ...
  if(possibleCenters_.size() < 3) {
    throw ImageException("no possible centers found");
  }

  vector< ImageRef <ImageQRFinderPattern> > patternInfo = selectBestPatterns();

  if(patternInfo.size() != 3) {
    throw ImageException("no pattern info found");
  }

  vector< ImageRef <ImageResultPoint> > patternInfoResPoints;

  for(size_t i=0; i<patternInfo.size(); i++)
      patternInfoResPoints.push_back(ImageRef<ImageResultPoint>(patternInfo[i]));

  ImageResultPoint::orderBestPatterns(patternInfoResPoints);

  patternInfo.clear();
  for(size_t i=0; i<patternInfoResPoints.size(); i++)
      patternInfo.push_back(ImageRef<ImageQRFinderPattern>(static_cast<ImageQRFinderPattern*>( &*patternInfoResPoints[i] )));

  ImageRef<ImageQRFinderPatternInfo> result(new ImageQRFinderPatternInfo(patternInfo));
  return result;
}

ImageRef<ImageBitMatrix> ImageQRFinderPatternFinder::getImage() {
  return image_;
}

vector<ImageRef<ImageQRFinderPattern> >& ImageQRFinderPatternFinder::getPossibleCenters() {
    return possibleCenters_;
}

bool ImageQRFinderPatternFinder::crossCheckDiagonal(int startI, int centerJ, int maxCount, int originalStateCountTotal) const
{
  int *stateCount = getCrossCheckStateCount();
...
int *ImageQRFinderPatternFinder::getCrossCheckStateCount() const
{
   memset(crossCheckStateCount, 0, sizeof(crossCheckStateCount));
   return crossCheckStateCount;
}

double ImageQRFinderPatternFinder::squaredDistance(ImageRef<ImageQRFinderPattern> a, ImageRef<ImageQRFinderPattern> b)
{
